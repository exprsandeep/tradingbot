...
class StructurePullbackStrategy(BaseStrategy):
    ...
    def should_enter_long(self, row: pd.Series, prev: pd.Series) -> bool:
        trend_long = bool(row["macro_close"] > row["macro_close_sma_50"] and row["macro_momentum_20"] > 0)
        pullback_long = bool(row["intermediate_pos20"] <= 0.40)
    ...
    def should_enter_short(self, row: pd.Series, prev: pd.Series) -> bool:
        trend_short = bool(row["macro_close"] < row["macro_close_sma_50"] and row["macro_momentum_20"] < 0)
        pullback_short = bool(row["intermediate_pos20"] >= 0.60)
