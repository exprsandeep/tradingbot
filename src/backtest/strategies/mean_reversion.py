...
class MeanReversionStrategy(BaseStrategy):
    ...
    def should_enter_long(self, row: pd.Series, prev: pd.Series) -> bool:
        # Oversold on 15m, downward momentum exhausted on 1h
        is_oversold = bool(row["s15_pos20"] <= 0.10)
        h1_mom_exhausted = bool(row["h1_momentum_20"] < -0.002)
        
        # 3m reversal sign (strong bullish bar)
    ...
    def should_enter_short(self, row: pd.Series, prev: pd.Series) -> bool:
        # Overbought on 15m, upward momentum exhausted on 1h
        is_overbought = bool(row["s15_pos20"] >= 0.90)
        h1_mom_exhausted = bool(row["h1_momentum_20"] > 0.002)
        
        # 3m reversal sign (strong bearish bar)
        reversal_bar = bool(row["close"] < row["open"] and row["body_pct_of_range"] < -0.6)
